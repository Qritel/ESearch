...
my_url = 'http://emploi-public.ma/fr/concoursListe.asp?c=0&e=1'

filename = "Concours/Etablissements_Entreprises_publics.csv"
f = open(filename, "a")
headers = " Id,\
...
for i in range(0, int(last_page) + 1):
    ...
    for row in table.findAll('tr'):
        ...
        s = cells[1].a["href"]
        f.write("concour_e1c0id" + re.findall("(?<=id=)(\d+)", s)[0] + ",")
        for j in range(2):
            f.write("\"" + cells[j].text.replace('\"', '-') + "\",")
        ...
        for j in range(6, 10):
            ...
            if len(files) != 0:
                ...
                f.write('-' + ',')
        if cells[1].a["href"] == 'concoursDetail.asp?c=0&e=1&id=5064' or cells[1].a["href"] == "concoursDetail.asp?c=0&e=1&id=4752":
            f.write('-,-\n')
            continue
